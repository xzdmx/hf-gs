...
max_sh_degree=0
def RGB2SH(rgb): # 将RGB值转换为球谐函数系数
    return (rgb - 0.5) / C0
def inverse_sigmoid(x): # 计算逆sigmoid函数
    return torch.log(x/(1-x))

def construct_list_of_attributes(features_dc,features_rest,scaling,rotation): # 构建属性列表，包含点的坐标、法线、特征、透明度、缩放和旋转信息
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        ...
        return l
def create_from_pcd(pcd,path): # 从点云对象 pcd 中提取点和颜色信息，进行一些计算（如深度距离、缩放、旋转、透明度等），并将所有信息保存为PLY文件
...
def similar_points_tree(point_cloud_A, point_cloud_B, threshold): # 使用KD树找到在阈值内相似的点
...
def composition(xyz_rgb, features_dc_rgb, features_extra_rgb, opacities_rgb, scales_rgb, rots_rgb,path_2): # 合并两个点云的属性
...
if __name__ == "__main__":
    ...
    # Remove the floaters point in the complemented area (这里用来删除初始残留在mask内的点)
