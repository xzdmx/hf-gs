...
python mask_edge_expend.py --input_dir ../data/${dataset}/colmap_dir/${scene}/seg \
      ...
      --size 40
## spin-nerf数据集不需要mask处理
#rm -r ../output/${dataset}/${scene}/mask/sub_mask/
#mkdir ../output/${dataset}/${scene}/mask/sub_mask/
#cp ../output/${dataset}/${scene}/mask/seg_expand/* ../output/${dataset}/${scene}/mask/sub_mask/

# mask处理
echo "---------------------------------------------mask 处理-------------------------------------------------------"
python get_inpaint_mask_2.py  --model_path ../output/${dataset}/${scene} \
      --last_iter 1 --expand_size 15
