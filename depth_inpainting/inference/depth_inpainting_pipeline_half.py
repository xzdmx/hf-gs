...
class DepthEstimationInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        # 对齐预测前的mask外深度
        depth_src = np.reciprocal(depth_numpy)
        depth = depth_align(depth_src, depth, mask)

        np.save(depth_path, depth)
        # 这一步保存对齐后的depth
        save_depth_as_colorful_image(depth, os.path.join(
            path_to_save, f"{rgb_name_base}_colored.png"
        ))
    ...
    def encode_depth(self, depth_latent: torch.Tensor) -> torch.Tensor:
        ...
        """
        # scale latent
        # assert not torch.any(torch.isnan(depth_latent))
...
def depth_align(real_depth, pred_depth, mask):
    # 读取mask图片并转换为numpy数组
    # mask = np.array(Image.open(mask_path).convert('L'))
    mask = mask > 0  # 将二值图转换为布尔数组
    ...
    # 计算mask外一圈的差值
    diff = real_depth[outer_border] - pred_depth[outer_border]
    # 确保diff是一个一维数组
    diff = np.array(diff)
    # 计算差值的平均值
    ...
    # # 绘制差值图
    # diff_map = real_depth - pred_depth
    ...
    # 将预测深度中mask内的部分加上平均差值进行调整
    pred_depth[mask] += avg_diff

    # 对齐后的深度，mask外直接使用真实的深度
    aligned_depth = np.where(~mask, real_depth, pred_depth)
