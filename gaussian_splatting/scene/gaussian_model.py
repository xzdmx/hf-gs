...
from utils.graphics_utils import z_score_from_percentage
...
class GaussianModel:
    ...
    def remove_large_points(self):
        # selected_pts_mask = (
        #     torch.where(torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent/2, True, False))

        # 获取torch.max(self.get_scaling, dim = 1).values的值
        max_scaling_values = torch.max(self.get_scaling, dim=1).values
        # 对值进行排序
        sorted_values, _ = torch.sort(max_scaling_values, descending=True)
        # 计算前5%的索引位置
        num_pts = len(sorted_values)
        num_remove = int(num_pts * 0.05)
        if num_remove == 0:
            threshold = sorted_values[0]
        else:
            threshold = sorted_values[num_remove - 1]
        # 根据阈值确定selected_pts_mask
        selected_pts_mask = (max_scaling_values > threshold)
        new_xyz = self._xyz[~selected_pts_mask]
        new_features_dc = self._features_dc[~selected_pts_mask]
        new_features_rest = self._features_rest[~selected_pts_mask]
